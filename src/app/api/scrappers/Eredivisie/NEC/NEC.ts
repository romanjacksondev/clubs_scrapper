// NEC Nijmegen official store (www.necshop.nl) — Shopify.
// The Shopify products.json API is blocked by bot-protection for server-side
// fetch; Puppeteer with stealth is used instead.
// Navigates to the homepage, discovers the kit collection link, then scrapes.

import { Product } from '../../PremierLeague/Product';
import { launchBrowser } from '../../PremierLeague/puppeteerUtils';

const STORE_BASE = 'https://www.necshop.nl';
...
const scrapeNEC = async (): Promise<Product[]> => {
  ...
  try {
    ...
    console.error('Error in scrapeNEC:', e);
